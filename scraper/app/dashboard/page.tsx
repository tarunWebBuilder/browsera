...
import { Play, ScanSearch, CalendarRange } from "lucide-react";
...
const WORKFLOW_TEMPLATES = [
  {
    ...
    defaults: {
      ...
      selectors: {
        ...
        submitSelector:
          "#menu3 > div.col-12.col-sm-12.col-md-12.col-xl-4 > button",
...
function EmptyState() {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (!openResp.ok || openData?.status === "failed") {
        throw new Error(
          openData?.error || openData?.detail || "Failed to open browser"
        );
      }
      const nextSessionId = openData?.result?.sessionId;
      if (!nextSessionId) {
      ...
      const selectors: string[] =
        data?.result?.forms
          ?.flatMap((f: any) => f.inputs?.map((i: any) => i.selector))
          ?.filter(Boolean) || [];
      ...
      const formFields = visibleInputs.reduce(
        (acc: Record<string, string>, input: any) => {
      ...
      const preferredCaptcha =
        ...
        captchaTypes.find((t) =>
          ["math", "number", "text", "image", "normal"].includes(t)
        ) ||
        "";
      const nextValues = {
        url: scanUrl,
        ...
        selectors: {
          ...
          captchaInputSelector:
            captchaSelectors?.inputSelector || captchaInput?.selector || "",
        },
      };
  ...
  return (
    <div className="flex w-full flex-col gap-6 rounded-xl border border-dashed p-6">
      <p className="text-center text-sm uppercase tracking-[0.2em] text-gray-400">
        Workflow templates
      </p>
          ...
          return (
            <button
              ...
              className={`w-full rounded-lg border px-4 py-4 text-left transition ${
                active
                  ? "border-orange-500 bg-[var(--card)] shadow-md"
                  : "border-[var(--border)] bg-[var(--panel)] hover:border-orange-500"
              } flex items-center gap-3`}
            >
              <div
                className={`flex h-10 w-10 items-center justify-center rounded-full ${
                  active ? "bg-orange-600" : "bg-[var(--panel-alt)]"
                }`}
              >
              ...
              {active && (
                <span className="rounded-full bg-orange-600 px-3 py-1 text-xs font-semibold text-black">
                  Selected
                </span>
              )}
            </button>
      ...
      <div className="space-y-2 rounded-lg border border-[var(--border)] bg-[var(--panel)] p-4">
        ...
        {foundSelectors.length > 0 && (
          <div className="text-xs text-muted">
            ...
            <div className="mt-1 flex flex-wrap gap-1">
              ...
              {foundSelectors.length > 12 && (
                <span className="px-2 py-1 text-gray-500">+ more</span>
              )}
            </div>
          </div>
        )}
      ...
      {formValues && (
        <div className="space-y-3 rounded-lg border border-border bg-(--panel) p-4">
              ...
              {dateFields.map((input: any) => (
                <div key={input.selector} className="flex flex-col gap-1">
                  <input
                    ...
                    onChange={(e) =>
                      updateField(`selectors.formFields.${input.selector}`, e.target.value)
                    }
                  />
          ...
          <div className="rounded bg-[var(--panel-alt)] p-2 text-xs text-[var(--muted)]">
          ...
          <button
            onClick={() => handleStart()}
            disabled={isStarting}
            className="w-full rounded-md bg-orange-600 px-5 py-2 text-sm font-medium text-black hover:bg-orange-500 disabled:opacity-60"
          >
